...
export default function NewSalePage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      router.push(`/sales/descarte/${created.id}`)
    } catch (err) {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <PageHeader
        ...
        description={`Paso ${step} de 2 — ${step === 1 ? 'Información general' : 'Seleccionar materiales'}`}
        breadcrumbs={[
          { label: 'Salidas', href: '/sales' },
          { label: 'Materiales de Descarte', href: '/sales/descarte' },
          { label: 'Nueva Oferta' },
        ]}
      />
      ...
      <div className="flex justify-between">
        <Button
          variant="outline"
          onClick={() => (step === 1 ? router.push('/sales/descarte') : setStep(1))}
        >
