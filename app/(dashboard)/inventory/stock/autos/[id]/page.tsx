...
import { useVehicleMutations } from '@/hooks/useStockItems'
...
import {
  Loader2, Pencil, Trash2, MapPin, Car, AlertTriangle, ArrowLeft,
} from 'lucide-react'
...
export default function VehicleDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: item, isLoading, error, refetch } = useQuery({
    queryKey: ['stockVehicle', id],
    queryFn: () => stockItemService.getVehicle(Number(id)),
  })
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center py-24">
        ...
        <span className="ml-2 text-slate-500">Cargando vehículo...</span>
      </div>
    )
  }
  ...
  if (error || !item) {
    return (
      <div className="text-center py-24">
        ...
        <p className="text-slate-500 font-medium">Vehículo no encontrado</p>
        <Button variant="outline" className="mt-4" onClick={() => router.push('/inventory/stock/autos')}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card>
          ...
          <CardContent className="grid grid-cols-2 gap-4">
            ...
            <DetailField label="Año" value={item.anio} />
            <DetailField label="Color" value={item.color} />
            <DetailField label="Placa" value={item.placa} />
            <DetailField label="Tipo de Vehículo" value={item.tipo_vehiculo} />
            <DetailField label="Tipo de Combustible" value={item.tipo_combustible} />
            <DetailField label="Tipo de Activo" value={item.tipo_activo} />
            <DetailField label="Número de Motor" value={item.numero_motor} />
            <DetailField label="Número de Chasis" value={item.numero_chasis} />
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-base">Fechas y Póliza</CardTitle>
          </CardHeader>
          <CardContent className="grid grid-cols-2 gap-4">
            <div>
              <p className="text-xs text-slate-500 mb-0.5">Fecha Revisado</p>
              <p className={`text-sm font-medium ${isExpiringSoon(item.fecha_revisado) ? 'text-orange-600' : 'text-slate-900'}`}>
                {isExpiringSoon(item.fecha_revisado) && <AlertTriangle className="h-3.5 w-3.5 inline mr-1" />}
                {formatDate(item.fecha_revisado)}
              </p>
            </div>
            <div>
              <p className="text-xs text-slate-500 mb-0.5">Vencimiento Póliza</p>
              <p className={`text-sm font-medium ${isExpiringSoon(item.fecha_vencimiento_poliza) ? 'text-orange-600' : 'text-slate-900'}`}>
            ...
            <DetailField label="Número de Póliza" value={item.numero_poliza} />
