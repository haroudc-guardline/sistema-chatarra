...
import { useToolMutations } from '@/hooks/useStockItems'
...
import {
  Loader2, Pencil, Trash2, MapPin, Wrench, ArrowLeft,
} from 'lucide-react'
...
export default function ToolDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: item, isLoading, error, refetch } = useQuery({
    queryKey: ['stockTool', id],
    queryFn: () => stockItemService.getTool(Number(id)),
  })
  ...
  const handleDelete = async () => {
    ...
    router.push('/inventory/stock/herramientas')
  }
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center py-24">
        ...
        <span className="ml-2 text-slate-500">Cargando herramienta...</span>
      </div>
    )
  }
  ...
  if (error || !item) {
    return (
      <div className="text-center py-24">
        ...
        <p className="text-slate-500 font-medium">Herramienta no encontrada</p>
        <Button variant="outline" className="mt-4" onClick={() => router.push('/inventory/stock/herramientas')}>
    ...
    <div className="space-y-6">
      <PageHeader
        ...
        breadcrumbs={[
          ...
          { label: 'Herramientas', href: '/inventory/stock/herramientas' },
          { label: item.nombre },
        ]}
        actions={
          (isAdmin || isOperador) ? [
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card>
          ...
          <CardContent className="grid grid-cols-2 gap-4">
            ...
            <DetailField label="Tipo" value={item.tipo_herramienta} />
            ...
            <DetailField label="Voltaje" value={item.voltaje} />
            <DetailField label="Potencia (Watts)" value={item.potencia_watts?.toLocaleString()} />
          </CardContent>
        </Card>
