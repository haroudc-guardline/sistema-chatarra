...
import type { Location, StockElectronic } from '@/types/database'

const schema = z.object({
  location_id: z.number().min(1, 'La institución es requerida'),
  nombre: z.string().min(1, 'El nombre es requerido'),
  tipo_electronico: z.string().optional(),
  marca: z.string().optional(),
  modelo: z.string().optional(),
  numero_serie: z.string().optional(),
  fecha_adquisicion: z.string().optional(),
  fecha_garantia: z.string().optional(),
...
interface AddElectronicDialogProps {
  open: boolean
  onOpenChange: (open: boolean) => void
  locations: Location[]
  onSuccess: () => void
  editItem?: StockElectronic
}

export function AddElectronicDialog({ open, onOpenChange, locations, onSuccess, editItem }: AddElectronicDialogProps) {
  ...
  const { register, handleSubmit, setValue, watch, reset, formState: { errors } } = useForm<FormData>({
    ...
    defaultValues: editItem
      ? {
          ...
          tipo_electronico: editItem.tipo_electronico || '',
          fecha_adquisicion: editItem.fecha_adquisicion || '',
          fecha_garantia: editItem.fecha_garantia || '',
          sistema_operativo: editItem.sistema_operativo || '',
  ...
  const watchedValues = {
    tipo_electronico: watch('tipo_electronico'),
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      let electronic: StockElectronic
      if (isEditing) {
        electronic = await stockItemService.updateElectronic(editItem!.id, cleaned)
      } else {
        electronic = await stockItemService.createElectronic(cleaned)
      }
      if (pendingFiles.length > 0) {
        await stockItemService.uploadPhotos('electronic', electronic.id, pendingFiles)
      }
      reset()
      setPendingFiles([])
      setPreviews([])
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{isEditing ? 'Editar Material Eléctrico' : 'Agregar Material Eléctrico'}</DialogTitle>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Input {...register('nombre')} placeholder="Laptop Dell Latitude" className={errors.nombre ? 'border-red-500' : ''} />
              {errors.nombre && <p className="text-xs text-red-500">{errors.nombre.message}</p>}
            </div>
            <div className="space-y-2">
              <Label>Tipo de Equipo Electrónico</Label>
              <Select value={watchedValues.tipo_electronico || ''} onValueChange={(v) => setValue('tipo_electronico', v)}>
                <SelectTrigger><SelectValue placeholder="Seleccionar tipo" /></SelectTrigger>
                <SelectContent>
                  {['Laptop','Computadora de Escritorio','Celular','Tablet','Impresora','Monitor','Proyector','Router','UPS','Otro'].map((t) => (
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className="space-y-2">
              <Label>Marca</Label>
              <Input {...register('marca')} placeholder="Dell" />
            </div>
            <div className="space-y-2">
              <Label>Modelo</Label>
              <Input {...register('modelo')} placeholder="Latitude 5520" />
            </div>
            <div className="space-y-2">
          ...
          <div className="space-y-2">
            ...
            <Input {...register('sistema_operativo')} placeholder="Windows 11, macOS, Android..." />
          </div>

          {/* Adquisición, Garantía, Frecuencia */}
          <div className="grid grid-cols-3 gap-4">
            <div className="space-y-2">
            ...
            <div className="space-y-2">
              ...
              <Input {...register('fecha_garantia')} type="date" />
            </div>
            <div className="space-y-2">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Input {...register('ubicacion_nombre')} placeholder="Oficina de TI" />
              </div>
              <div className="space-y-2">
          ...
          <DialogFooter>
            ...
            <Button type="submit" disabled={isSubmitting}>
              ...
              {isEditing ? 'Guardar Cambios' : 'Agregar Material Eléctrico'}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
